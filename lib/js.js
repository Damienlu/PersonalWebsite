function () {
    var x = document.getElementByClassName("image-title");
    x.style.opacity = 1;
}
...
var imageParallax = function () {
    ...
    $('.contact__page').css('background-position', 'center calc(150% + ' + (_NOW * 0.3) + 'px');
    if(_SMW > 768) {
        $('.image-title').css({'opacity': aa, 'padding-top': ab + 'vh'});
    } else {
        $('.image-title').css({'opacity': aa, 'padding-top': ac + 'vh'});
    }
};
